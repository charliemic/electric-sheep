...
### Security Principle: Read-Only, Minimal Scope

**CRITICAL**: AWS access should be read-only and scoped only to Bedrock model usage.

### Minimum Required Permissions (Read-Only)

Your AWS IAM user/role needs these permissions - **read-only access only**:
...
- Invoke Bedrock models for inference
...
- Creating or deleting Bedrock resources
- Accessing other AWS services (S3, EC2, Lambda, etc.)
- Reading or writing to other AWS resources
- Any write operations on Bedrock

**Security Note**: This policy follows the principle of least privilege - it only grants the minimum permissions needed to use Bedrock models, nothing more.
...
- ✅ Credentials are in `.gitignore` - verify they're not tracked
- ✅ Use temporary SSO credentials when possible
- ✅ Rotate access keys if accidentally exposed

**Minimal permissions:**
- ✅ Use IAM users/roles with **read-only** Bedrock permissions only
- ✅ Follow principle of least privilege
- ✅ No write permissions unless absolutely necessary
- ✅ Scope restricted to Bedrock model usage only

**Credential management:**
- ✅ Use AWS SSO for temporary credentials
- ✅ Extract credentials using scripts (credentials not in scripts)
- ✅ Store credentials locally only (never in repo)
